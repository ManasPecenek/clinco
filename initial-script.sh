...
then
  export KUBERNETES_PUBLIC_ADDRESS=$(hostname -I)  #172.17.0.1 #host.docker.internal #$(hostname)
fi
...
docker exec -i --privileged --user root master bash -c "./worker.sh $NODE_COUNT $j" #> /dev/null
...
then
  ...
  kubectl apply -k kube-tools -k cluster/flux-system
